...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "تم تسجيل الدخول", description: "مرحباً بك مرة أخرى في THREAD." });
      router.push('/');
    } catch (error: any) {
      toast({
        variant: "destructive",
    ...
    <div className="container mx-auto px-4 py-20 flex justify-center items-center min-h-[80vh]">
      <Card className="w-full max-w-md shadow-2xl rounded-3xl overflow-hidden border-none bg-white">
        ...
        <CardContent className="space-y-6 px-8 pb-10">
          ...
          <Button variant="outline" className="w-full h-12 rounded-xl gap-3" onClick={handleGoogleLogin}>
          ...
          <p className="text-center text-sm text-muted-foreground mt-4">
            ليس لديك حساب؟ <Link href="/signup" className="text-accent hover:underline font-bold">إنشاء حساب جديد</Link>
          </p>
        </CardContent>
      </Card>
    </div>
